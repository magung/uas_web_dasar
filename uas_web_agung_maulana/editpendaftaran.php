...
<body>
	<?php 
		...
		$id = date("YmdHis");
		$d = new Database(); 
		$iddaftar = (!isset($_REQUEST["iddaftar"]))? null : $_REQUEST["iddaftar"];

		$sqlpendaftaran = "SELECT pd.*, ps.namapasien, ps.kontak, ps.alamat, ps.foto FROM `pendaftaran` as pd INNER JOIN `pasien` as ps on ps.idpasien = pd.idpasien WHERE iddaftar = '".$iddaftar."'";
	...
	<div>
		<form action="<?php $_SERVER["PHP_SELF"] ?>" method="POST" enctype="multipart/form-data">
			...
			<div>
				...
				<a href="editpendaftaran.php?action=hapus_foto&iddaftar=<?php echo $_REQUEST["iddaftar"];?>">Hapus Foto</a>
			</div>
			<div>
			...
			<div>
				...
				<a href="datapendaftaran.php">Batal</a>
			</div>
		</form>
	...
	<?php 
			...
			$d->query($sqlpasien);
			header("location: editpendaftaran.php?iddaftar=".$_REQUEST["iddaftar"]);
		}
	?>
</body>
</html>